    ...
    def generate_stream_gate(self, params):
        ...
        try:
            ...
            for output in self.generate_stream_func(
                self.model, self.tokenizer, params, self.device, self.context_len, self.stream_interval,
            ):
            ...
            if self.embed_in_truncate:
                ...
                for i in range(0, input_ids.size(1), self.context_len):
                    ...
                    if hasattr(self.model, "use_cls_pooling") and self.model.use_cls_pooling:
                        cls_tokens = (
                            torch.zeros(
                                (chunk_input_ids.size(0), 1), dtype=chunk_input_ids.dtype, device=chunk_input_ids.device,
                            ) + tokenizer.cls_token_id
                        )
                        chunk_input_ids = torch.cat([cls_tokens, chunk_input_ids], dim=-1)
                        mask = torch.ones(
                            (chunk_attention_mask.size(0), 1), dtype=chunk_attention_mask.dtype, device=chunk_attention_mask.device,
                        )
...
def create_model_worker():
    ...
    parser.add_argument("--port", type=int, default=10082)
    parser.add_argument("--worker-address", type=str, default="http://localhost:10082")
